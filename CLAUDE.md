...
- **Edge Function `support-chat`**: recebe o histórico da conversa (JWT do profissional logado, sem `--no-verify-jwt` — precisa de sessão real), monta o system prompt com as regras de tom/segurança/escalação/conhecimento de produto do `contexto-ia-suporte.md`, e roda o loop de tool-use inteiro (chamar a Claude → executar a RPC certa → devolver o resultado → repetir até a Claude parar de pedir ferramenta) **dentro da própria invocação** — o cliente nunca vê nem reenvia blocos de `tool_use`/`tool_result`, decisão deliberada pra não ter que confiar no navegador pra ecoar de volta sem adulterar um resultado que só o servidor deveria ter gerado. Teto de 30 mensagens do usuário por conversa (seção 7 do doc) checado antes de chamar a API — se estourar, responde com a orientação de escalar sem gastar token. Implantada em produção.
- **`support-widget.js`** (novo arquivo na raiz, incluído via `<script src="support-widget.js" defer">` nas 7 páginas): botão flutuante + painel de chat, autocontido (cria o próprio client Supabase, não depende do `supa` de cada página) — reaproveita os tokens de design (`--primary`, `--card-grad`, `--radius-lg` etc.) já definidos em cada página, sem redefinir paleta. Detecta `.bottom-nav` pra ajustar a posição do botão (as 3 páginas que usam `#studentTabbar` em vez de nav inferior não têm esse elemento). Histórico da conversa persiste só em `sessionStorage` (esvazia ao fechar a aba) — decisão deliberada de não criar tabela nova pra isso, consistente com o design stateless do backend.
...
- **Testado ponta a ponta em produção**: usando uma sessão de teste pré-existente no navegador (conta allowlist do `dev-generate-login`, `doug.sickmind@gmail.com`, plano Starter/trial) — pergunta geral de produto (explicou os dois jeitos de criar treino corretamente), chamada real de `get_my_account_status` (relatou plano/limite de alunos/data de fim do trial batendo com o banco), chamada real de `check_student_protocol_status` pra um aluno inexistente (respondeu corretamente que não encontrou e sugeriu os próximos passos certos, incluindo o caminho de escalação por confusão de tenant). Não testado ainda: o teto de 30 mensagens (exigiria 31 turnos manuais) e o caso de um aluno que *existe* de fato — ambos de baixo risco (lógica simples, sem tool nova envolvida), mas vale confirmar num uso real.
- **Pendências que ficam pro usuário validar em uso real**: revisar o texto exato das respostas em mais perguntas do dia a dia (a seção 6 do `contexto-ia-suporte.md` lista só um ponto de partida, não uma lista fechada) e observar o custo real por conversa no painel da Anthropic depois de algum uso — a decisão de não ter rate-limit por profissional (seção 7 do doc) foi deliberadamente adiada até aparecer tração real.
