...
1. Criar a Aplicação no Mercado Pago (`developers.mercadopago.com.br`, mesma conta pessoal do usuário serve — ver ressalva sobre migrar pro CNPJ/MEI mais adiante, não bloqueia agora). Gerar credenciais de teste primeiro (Public Key + Access Token), depois produção.
2. Configurar o webhook na aplicação do Mercado Pago apontando pra URL da função `mercadopago-webhook` (só existe depois do deploy) e copiar a "chave secreta" gerada lá.
3. Rodar `supabase secrets set MERCADOPAGO_ACCESS_TOKEN=... MERCADOPAGO_WEBHOOK_SECRET=...` e aplicar `supabase_14_billing.sql` — **só funciona localmente** (notebook), pela limitação de proxy já documentada abaixo ("Limitação de rede descoberta em sessão anterior").
4. Deploy das duas functions: `supabase functions deploy mercadopago-webhook --no-verify-jwt` e `supabase functions deploy mercadopago-create-preapproval` — idem, só local.
5. Trocar `MERCADOPAGO_PUBLIC_KEY` em `onboarding.html` (hoje um placeholder `SUBSTITUIR_PELA_PUBLIC_KEY_MERCADO_PAGO`) pela Public Key real.
6. Testar o fluxo completo em sandbox: criar "conta de teste" no painel do Mercado Pago, usar um dos números de cartão de teste publicados por eles (não precisa do cartão real do usuário) e confirmar que o onboarding cria a assinatura, o webhook chega, a assinatura valida e `professionals` é atualizado.
7. Fluxo de cancelamento/dunning (o que fazer quando um pagamento recorrente é recusado repetidas vezes) é o item 4 da Fase A, ainda não construído — o webhook hoje só registra `ultima_cobranca_status` quando um pagamento é rejeitado, não toma nenhuma ação automática sobre isso.
