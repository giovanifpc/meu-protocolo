...
Treinos: manual (profissional monta do zero — título, periodização, treinos A/B/C, busca de ~1550 exercícios com GIF, séries/reps/descanso) ou por IA (pergunta objetivo/nível/periodização/frequência/duração, gera o protocolo inteiro considerando a anamnese de saúde e o gênero do aluno automaticamente — evita exercício fora de contexto e calibra ênfase, sem estereotipar —, cai na tela de edição pra revisão — nada publica sozinho). Periodização calcula sozinho a evolução de sets/reps/descanso semana a semana. Cardio pode ser adicionado como item especial, sempre por último no treino. Protocolo é rascunho (só profissional vê) ou publicado (aluno já enxerga e pode treinar). Criar um treino novo por cima de um já existente não apaga o anterior — ele vira rascunho, acessível num dropdown "Rascunhos anteriores deste aluno" (reabrir ou apagar de vez).
