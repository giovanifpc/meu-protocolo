...
O profissional registra medidas (dobras cutâneas, bioimpedância, perimetria, fotos) periodicamente — o app calcula % de gordura e mostra evolução comparando com a avaliação anterior. É tudo digitado manualmente pelo profissional (nenhuma balança do mercado tem integração direta). Fotos: ao escolher, aparece um preview na hora (a foto é comprimida automaticamente antes de subir, então funciona mesmo com foto grande de câmera de celular); formatos aceitos são JPEG/PNG/WEBP — se a foto vier em HEIC (comum em iPhone) e o app avisar que não conseguiu abrir, oriente exportar/converter pra JPEG antes de tentar de novo. As fotos aparecem em miniatura tanto no painel do profissional quanto na tela de Avaliação física do aluno, e dá pra remover uma foto sem precisar substituir por outra (botão "Remover foto").

**Comparativos**: com 2 ou mais avaliações finalizadas, o profissional pode criar um "Comparativo" (botão dentro de Avaliação física) — escolhe qual avaliação é "Antes" e qual é "Depois", e o app monta sozinho as fotos lado a lado por ângulo (frente/lado/costas) e uma tabela com a variação de cada medida (peso, % gordura, perimetria). Fica como rascunho, só o profissional vê, até ele clicar "Publicar pro aluno" — só depois disso aparece pra ela na tela de Avaliação física dela também. Dá pra despublicar (some da tela do aluno, mas o comparativo continua existindo) ou excluir de vez. Excluir/editar um comparativo nunca apaga nem altera as avaliações originais usadas nele.
...
Na aba Nutri do aluno, abaixo da orientação em texto/PDF de sempre (seção 5.8, que continua existindo do mesmo jeito — os dois blocos convivem, um nunca substitui o outro), tem um diário alimentar de verdade: o aluno busca um alimento (banco de ~1680 itens, TACO + industrializados) ou cria um item próprio quando não acha, escolhe a refeição (café da manhã, lanche, almoço etc.) e a quantidade em gramas — o app calcula sozinho as calorias e macros e mostra o total do dia. Isso é **medição**, disponível em **todo plano** desde o primeiro dia, sem precisar de nada configurado pelo profissional. **Não é só "hoje" (desde 2026-08-11)**: o próprio aluno tem o mesmo seletor de dia da semana + setas de navegar semana — dá pra ver e até registrar/editar/remover refeição de um dia passado (esqueceu de anotar o almoço de ontem? dá pra lançar retroativo, com um aviso na hora deixando claro que é pra um dia diferente de hoje). Reentrar na aba Nutri sempre volta o diário pra hoje, nunca fica "preso" num dia antigo.
