...
- **Fluxo de cancelamento** (item 4 da Fase A): `perfil.html` ganhou card "Cancelar assinatura" com tela de confirmação (fim de acesso, retenção de 30 dias, aviso de exclusão permanente) — muda `professionals.status` pra `'inativo'` + grava `inactive_since`, desloga. `login.html` bloqueia um novo login enquanto `status = 'inativo'` (mensagem explicando o motivo, em vez de deixar entrar). Testado ponta a ponta em produção com conta de teste. **Falta**: chamar a API do Mercado Pago pra cancelar a assinatura de verdade (só existe quando o item 1 estiver funcionando).
