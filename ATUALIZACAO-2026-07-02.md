> **Nota (2026-07-03):** este documento ficou numa branch (`claude/context-sync-devices-6p3tup`) que nunca foi mesclada na `main` — por isso não aparecia em sessões seguintes. Trazido agora como registro histórico. O "bug novo encontrado" descrito abaixo (loop onboarding ↔ painel por RLS) **já foi corrigido** em commits posteriores na `main` (`e153f99`, `69d6211`, `89c2b1d`). O token de acesso pessoal do Supabase mencionado nas notas operacionais já foi revogado.
