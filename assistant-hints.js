...
(function () {
  ...
  function render(hint) {
    ...
    card.querySelectorAll('.hint-btn').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        hide();
        // espera o registro terminar antes de navegar — navegar embora
        // cancelaria a requisição em voo antes dela chegar no servidor.
  ...
  async function checkHint() {
    ...
    checking = true;
    try {
      const { data, error } = await supa.rpc('get_next_hint').maybeSingle();
