...
- Como executar um treino: marcar séries, registrar carga/reps, o timer de descanso automático, o resumo final com avaliação de humor. Cada série já abre pré-preenchida com a carga e os reps que você realmente fez naquela MESMA série (1ª com a 1ª, 2ª com a 2ª...) na última vez que treinou esse exercício — nunca um número único repetido em todas as séries; é só editar se quiser mudar. Se a máquina do próximo exercício da sequência estiver ocupada, o botão "Lista" (rodapé da tela de execução) mostra todos os exercícios do treino com check nos já concluídos — dá pra tocar em qualquer um pra pular direto pra ele, e "Próximo" passa a pular automaticamente quem já foi feito.
- Como ver evolução de carga, histórico de treinos, avaliação física (se o personal já registrou alguma).
- Como funciona a aba Nutri: orientação em texto + PDF que o personal sobe, e o diário alimentar (registrar o que comeu, buscar ou criar um alimento, ver o total do dia) — isso funciona sempre, mesmo sem o personal ter configurado nada. A meta de calorias/macros só aparece depois que o personal preenche os dados de uma consulta real com nutricionista parceira (é exigência legal, não falha do app) — sem isso, o diário continua funcionando normal, só sem barra de progresso. O diário não é só de hoje: tem um seletor de dia da semana + setas pra navegar entre semanas, e dá pra registrar/editar/remover refeição de um dia passado (esqueceu de anotar? lança retroativo) — a aba sempre volta pra hoje sozinha ao reabrir.
