...
- **Pendências que ficam pro usuário validar em uso real**: revisar o texto exato das respostas em mais perguntas do dia a dia (a seção 6 do `contexto-ia-suporte.md` lista só um ponto de partida, não uma lista fechada); observar o custo real por conversa no painel da Anthropic depois de algum uso (rate-limit por profissional deliberadamente adiado até aparecer tração real); dar uma olhada visual na seção "Conversas de suporte" do `master.html` pra confirmar que o layout está bom; e o teto de 30 mensagens por conversa não foi testado ao vivo (exigiria 31 turnos manuais, lógica simples e de baixo risco).
